...
(defun chun-bili--get-ch-en-content-at-this-point ()
  ...
  (let* ((root (org-element-at-point))
         (content-elements (org-element--parse-elements
                            (org-element-property :contents-begin root)
                            (org-element-property :contents-end root) nil nil nil nil nil))
         (res (ht-create))
         )
    (org-element-map content-elements 'headline
      (message "children: %S" content-elements)
      (lambda (headline)
        (let* (
               (title (org-element-property :raw-value headline))
               (content (buffer-substring-no-properties (org-element-property :contents-begin headline)
                                                        (org-element-property :contents-end headline)
                                                        )))
          (message "visiting element: %S" headline)

          (when (string-equal title "En")
            (ht-set! res "en" content))
          (when (string-equal title "Ch")
            (ht-set! res "ch" content)))))
    res
    ))

  (defun chun-bili-get-content-at-this-point ()
    (interactive)
    (message "%S" (chun-bili--get-ch-en-content-at-this-point))
    )
#+END_SRC
