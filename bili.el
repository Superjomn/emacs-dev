...
(defconst chun-bili--note-type "Basic (and reversed card)")
(defconst chun-bili--dock "English-translate")
(defconst chun-bili--en "En")
(defconst chun-bili--ch "Ch")
    ...
    (save-excursion
      (with-temp-buffer
        ...
        (org-element-map content-element
            'headline
          (lambda (elem)
            (let* ((elem-title (org-element-property :raw-value elem)) elem-content)
...
(defun chun-bili--pair-data-to-org (lines0 lines1 note-type anki-dock headline-level)
...
(defun chun-bili--write-to-file (content path)
  ...
  (save-excursion
    (let*
        ((buffer (get-buffer-create "*CHUN-BILI*")))
...
(defun chun-bili--pair-data-this ()
  ...
  (let* ((pair-dic (chun-bili--get-ch-en-content-at-this-point))
         ...
         (lines-en (split-string content-en "\n"))
         (lines-ch (split-string content-ch "\n"))
         (element (org-element-at-point))
         (anki-properties (chun-bili--get-anki-properties element))
         (anki-note-type (car anki-properties))
         (anki-dock (cdr anki-properties))
         org-content
         )

    (setq org-content (chun-bili--pair-data-to-org lines-en lines-ch anki-note-type
                                      anki-dock 1))
    (message "%S" org-content)
    (chun-bili--write-to-file org-content chun-bili-publish-file)))

(chun-bili--create-anki-org-record "line0" "line1" "node-type" "en" 3)
(chun-bili--pair-data-to-org '("a" "b") '("c" "d") "note-type" "anki-dock" 2)
